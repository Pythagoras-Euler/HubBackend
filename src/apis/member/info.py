...
async def get_banner(request: Request, response: Response,
    userid: Optional[int] = None, uid: Optional[int] = None, discordid: Optional[int] = None, steamid: Optional[int] = None, truckersmpid: Optional[int] = None, language: Optional[str] = None):
    """Returns the banner generated by [BannerGen]"""
    app = request.app
    dhrid = request.state.dhrid
    await app.db.new_conn(dhrid, db_name = app.config.db_name)

    if language is not None and language not in ("en", "zh", "de", "es"):
        response.status_code = 422
        return {"error": "Unsupported banner language"}
    ...
    if userid in [-1, None]:
        return RedirectResponse(url=f"{app.config.prefix}/member/banner?userid={t[0][5]}{language_query}", status_code=302)

    for param in request.query_params:
        if param not in ("userid", "language"):
            return RedirectResponse(url=f"{app.config.prefix}/member/banner?userid={userid}{language_query}", status_code=302)
